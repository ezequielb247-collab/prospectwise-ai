...
export class SupabasePersistenceRepository implements PersistentRepository {
  constructor(
  ...
  async updateCampaignStatus(
    ...
    const { error: activityError } = await this.client
      ...
      .insert({
        ...
        note: paused ? "Campanha pausada." : "Campanha retomada.",
      });
  ...
  async moveLead(input: Parameters<PersistentRepository["moveLead"]>[0]) {
    const { data, error } = await this.client
      .from("leads")
      .update({ crm_stage: input.stage, updated_at: new Date().toISOString() })
      .eq("user_id", this.userId)
      .eq("id", input.leadId)
